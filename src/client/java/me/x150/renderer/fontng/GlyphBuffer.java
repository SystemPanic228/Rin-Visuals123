...
public class GlyphBuffer {
	...
	/**
	 * Глобальный множитель прозрачности текста (0..1).
	 * Значение 1 полностью сохраняет прежнее поведение — глифы рисуются с непрозрачной альфой.
	 * Клик-гуи выставляет его на время анимаций открытия/закрытия окна и появления элементов.
	 */
	public static float alphaMultiplier = 1.0f;

	private static int glyphAlphaBits() {
		float m = alphaMultiplier;
		if (!(m >= 0f)) m = 0f;
		if (m > 1f) m = 1f;
		return Math.round(255f * m) << 24;
	}
	...
	public void draw(DrawContext context, float x, float y) {
		...
		// Альфу фиксируем ЗДЕСЬ, а не внутри лямбды: вершины набиваются
		// отложенно, при сбросе кадра, и статическое поле к тому моменту
		// уже могло изменить значение.
		final int alphaBits = glyphAlphaBits();
...
//			context.fill((int) ((minX + offsetX) * sf), (int) ((minY + offsetY) * sf), (int) (((minX + offsetX) * sf) + (maxX - minX) * sf), (int) (((minY + offsetY) * sf) + (maxY - minY) * sf), 0xFFFF00FF);
			SimpleGuiElementRenderState state = new SimpleGuiRenderState(CustomRenderLayers.PIPELINE_TEXT_CUSTOM, TextureSetup.of(glId), context, theBounds, (buffer) -> {
				for (Glyph glyph : glyphPageListEntry.getValue()) {
					...
					int actualColor = ((textCol == null ? 0xFFFFFF : textCol.getRgb()) & 0x00FFFFFF) | alphaBits;
			...
			SimpleGuiElementRenderState state = new SimpleGuiRenderState(RenderPipelines.GUI, TextureSetup.empty(), context, theBounds, (quadBuffer) -> {
				for (List<Rectangle> whatToDraw : draws) {
					for (Rectangle rect : whatToDraw) {
						...
						int actualColor = 0x00FFFFFF | alphaBits;
						if (rect.color != null) actualColor = (rect.color.getRgb() & 0x00FFFFFF) | alphaBits;
